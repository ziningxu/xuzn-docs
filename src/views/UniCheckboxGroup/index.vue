<template>
  <UniExample :exp-keys="ks">
    <template #pre>
      <h1>UniCheckboxGroup 多选框</h1>
      <p>
        <ElText> 此表格完全继承自 </ElText>
        <ElText type="warning"> ElCheckboxGroup </ElText>
        <ElText> 拥有其所有的属性，插槽，事件 </ElText>
      </p>
    </template>
    <div>
      <h2 data-id>UniCheckboxGroup Api</h2>
      <ElAlert type="warning">完全继承自ElCheckboxGroup，相同的属性就不做介绍了</ElAlert>
...
const ks = ['UniCheckboxGroup1']

const attrData: AttrListItemType[] = [
  { attr: 'options', desc: '方便的使用数组展示，兼容原生，如果没有options，会读取默认插槽', type: 'UniCheckboxPropsType[]', default: '--' },
]
</script>
